    ...
    public ArrayList<UserDataDTO> search(UserDataDTO ud) throws SQLException{
        ...
        try{
            ...
            //if文で中身がない時の処理を記述
                        int m = 1;
            if(!ud.getName().equals("")){
                                st.setString(m, "%"+ud.getName()+"%");
                                m+=1;          
            }
            if(ud.getBirthday() != null){
                                st.setString(m, "%"+ new SimpleDateFormat("yyyy").format(ud.getBirthday())+"%");
                                m+=1;                      
            }
            if(ud.getType() != 0){
                                st.setInt(m, ud.getType());
                               
            }
            
            ResultSet rs = st.executeQuery();
            ArrayList<UserDataDTO> dtoList = new ArrayList();
            while(rs.next()){
